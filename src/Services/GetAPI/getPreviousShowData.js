...
const getPreviousShowData = (data, update = false) => {
    return function (dispatch) {
        if (!data.cacheData.data.teamData.isAlreadyCalled || update) {
            dispatch(updateLoader(true));
            const config = axiosConfigReturn('about/previousShow', 'get');
            axios(config).then((response) => {
                dispatch(updateLoader(false));
                if (response?.data?.type === 'success') {
                    const storeData = {
                        key: 'previousShowData',
                        value: {
...
export default getPreviousShowData;